...
public class PolynomialNode2 extends AbstractNode implements KPolynomialNode {
    ...
    public PolynomialNode2(long p_world, long p_time, long p_id, KGraph p_graph, long[] currentResolution) {
    ...
    public void set(double value) {
        ...
        KResolver.KNodeState phasedState = graph().resolver().newState(this, world(), time()); //force clone
        //KResolver.KNodeState phasedState = graph().resolver().resolveState(this, false); //force clone

        phasedState.set(PRECISION_KEY, KType.DOUBLE, 0.1);
        phasedState.set(WEIGHT_KEY, KType.DOUBLE_ARRAY, new double[]{value});
        phasedState.set(NB_PAST_KEY, KType.INT, 2);
        phasedState.set(STEP_KEY, KType.LONG, 1);
        phasedState.set(LAST_TIME_KEY, KType.LONG, time());

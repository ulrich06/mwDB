...
public class Gaussian1D {
    ...
    public static double getVariance(double sum, double sumSq, int total){
        return (sumSq-(sum*sum)/total)/(total-1);
    }
    public static double getProbability(double sum, double sumSq, int total, double feature){

        if(total<2){
            return 0;
        }
        double avg=sum/total;
        double cov=getVariance(sum,sumSq,total);

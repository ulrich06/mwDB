    ...
    (function (mwg) {
        ...
        (function (plugin) {
            var AbstractNode = (function () {
                ...
                AbstractNode.prototype.findAt = function (indexName, world, time, query, callback) {
                    ...
                    if (indexMap != null) {
                        ...
                        var flatQuery = org.mwg.plugin.Query.parseQuery(query, selfPointer._resolver);
                        var foundId = indexMap.get(flatQuery.hash());
                        if (foundId == null) {
                            callback(new Array(0));
                            return;
                        }
                        ...
                        for (var i = 0; i < foundId.length; i++) {
                            selfPointer._resolver.lookup(world, time, foundId[i], function (resolvedNode) {
                        ...
                        waiter.then(function (o) {
                            ...
                            for (var i = 0; i < resultSet.length; i++) {
                                ...
                                var exact = true;
                                for (var j = 0; j < flatQuery.size; j++) {
                                    var obj = resolvedState.get(flatQuery.attributes[j]);
                                    if (flatQuery.values[j] == null) {
                                        ...
                                        if (obj == null) {
                                            ...
                                            if (!org.mwg.Constants.equals(flatQuery.values[j], obj.toString())) {
                                                exact = false;
                                                break;
                                            }
                                        }
                ...
                AbstractNode.prototype.find = function (indexName, query, callback) {
                    this.findAt(indexName, this.time(), this.world(), query, callback);
                };
                AbstractNode.prototype.allAt = function (indexName, world, time, callback) {
                    var currentNodeState = this._resolver.resolveState(this, false);
                    if (currentNodeState == null) {
                        throw new Error(org.mwg.Constants.CACHE_MISS_ERROR);
                    }
                ...
                AbstractNode.prototype.all = function (indexName, callback) {
                    this.allAt(indexName, this.world(), this.time(), callback);
                };
                AbstractNode.prototype.index = function (indexName, nodeToIndex, flatKeyAttributes, callback) {
                    ...
                    var flatQuery = new org.mwg.plugin.Query();
                    var toIndexNodeState = this._resolver.resolveState(nodeToIndex, true);
                    for (var i = 0; i < keyAttributes.length; i++) {
                        var attKey = this._resolver.stringToHash(keyAttributes[i], true);
                        var attValue = toIndexNodeState.get(attKey);
                        if (attValue != null) {
                ...
                AbstractNode.prototype.unindex = function (indexName, nodeToIndex, flatKeyAttributes, callback) {
                    ...
                    if (indexMap != null) {
                        ...
                        var toIndexNodeState = this._resolver.resolveState(nodeToIndex, true);
                        for (var i = 0; i < keyAttributes.length; i++) {
                            var attKey = this._resolver.stringToHash(keyAttributes[i], false);
                            var attValue = toIndexNodeState.get(attKey);
                            if (attValue != null) {
                                flatQuery.add(attKey, attValue.toString());
                            }
                ...
                AbstractNode.prototype.setPropertyWithType = function (propertyName, propertyType, propertyValue, propertyTargetType) {
                    if (propertyType != propertyTargetType) {
                        throw new Error("Property " + propertyName + " has a type mismatch, provided " + org.mwg.Type.typeName(propertyType) + " expected: " + org.mwg.Type.typeName(propertyTargetType));
                    }
                    else {
                        var preciseState = this._resolver.resolveState(this, false);
                        if (preciseState != null) {
            ...
            var Query = (function () {
                function Query() {
                    this.capacity = 1;
                    this.attributes = new Float64Array(this.capacity);
                ...
                Query.prototype.add = function (att, val) {
                    if (this.size == this.capacity) {
                        ...
                        java.lang.System.arraycopy(this.attributes, 0, temp_attributes, 0, this.capacity);
                        java.lang.System.arraycopy(this.values, 0, temp_values, 0, this.capacity);
                        this.attributes = temp_attributes;
                        this.values = temp_values;
                        this.capacity = temp_capacity;
                    }
                    this.attributes[this.size] = att;
                    this.values[this.size] = val.trim();
                    this.size++;
                };
                Query.prototype.compute = function () {
                    this.sort();
                    this._hash = 0;
                ...
                Query.prototype.sort = function () {
                    for (var i = (this.size - 1); i >= 0; i--) {
                        for (var j = 1; j <= i; j++) {
                            if (this.attributes[j - 1] > this.attributes[j]) {
                                var tempK = this.attributes[j - 1];
                                var tempV = this.values[j - 1];
                                this.attributes[j - 1] = this.attributes[j];
                                this.values[j - 1] = this.values[j];
                                this.attributes[j] = tempK;
                                this.values[j] = tempV;
                            }
                        }
                    }
                };
                Query.parseQuery = function (query, p_resolver) {
                    var cursor = 0;
                    var currentKey = org.mwg.Constants.NULL_LONG;
                    var lastElemStart = 0;
                    var flatQuery = new org.mwg.plugin.Query();
                    while (cursor < query.length) {
                        if (query.charAt(cursor) == org.mwg.Constants.QUERY_KV_SEP) {
                            if (lastElemStart != -1) {
                            ...
                            if (query.charAt(cursor) == org.mwg.Constants.QUERY_SEP) {
    ...
    (function (mwg) {
        ...
        (function (core) {
            ...
            var Builder = (function () {
                ...
                Builder.prototype.newGraph = function (p_storage, p_readOnly, p_scheduler, p_factories, p_usingGC, p_usingOffHeapMemory, p_memorySize, p_autoSaveSize) {
                    ...
                    var graph = new org.mwg.core.CoreGraph(storage, space, scheduler, new org.mwg.core.MWGResolver(storage, space, nodeTracker, scheduler), p_factories);
                    ...
                    var HeapTimeTreeChunk = (function () {
                        ...
                        HeapTimeTreeChunk.prototype.clearAt = function (max) {
                            ...
                            try {
                                ...
                                this._back_meta = new Int32Array(this._back_k.length * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE);
                                this._back_colors = [];
                                this._root_index = -1;
                        ...
                        HeapTimeTreeChunk.prototype.allocate = function (capacity) {
                            this._back_meta = new Int32Array(capacity * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE);
                        ...
                        HeapTimeTreeChunk.prototype.reallocate = function (newCapacity) {
                            ...
                            var new_back_meta = new Int32Array(newCapacity * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE);
                            if (this._back_meta != null) {
                                java.lang.System.arraycopy(this._back_meta, 0, new_back_meta, 0, this._size * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE);
                                for (var i = this._size * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE; i < newCapacity * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE; i++) {
                                    new_back_meta[i] = -1;
                                }
                            }
                        ...
                        HeapTimeTreeChunk.prototype.left = function (p_currentIndex) {
                            ...
                            return this._back_meta[p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE];
                        };
                        HeapTimeTreeChunk.prototype.setLeft = function (p_currentIndex, p_paramIndex) {
                            this._back_meta[p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE] = p_paramIndex;
                        };
                        HeapTimeTreeChunk.prototype.right = function (p_currentIndex) {
                            if (p_currentIndex == -1) {
                                return -1;
                            }
                            return this._back_meta[(p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE) + 1];
                        };
                        HeapTimeTreeChunk.prototype.setRight = function (p_currentIndex, p_paramIndex) {
                            this._back_meta[(p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE) + 1] = p_paramIndex;
                        };
                        HeapTimeTreeChunk.prototype.parent = function (p_currentIndex) {
                            if (p_currentIndex == -1) {
                                return -1;
                            }
                            return this._back_meta[(p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE) + 2];
                        };
                        HeapTimeTreeChunk.prototype.setParent = function (p_currentIndex, p_paramIndex) {
                            this._back_meta[(p_currentIndex * org.mwg.core.chunk.heap.HeapTimeTreeChunk.META_SIZE) + 2] = p_paramIndex;
                        };
            ...
            var CoreGraph = (function () {
                function CoreGraph(p_storage, p_space, p_scheduler, p_resolver, p_factories) {
                    ...
                    if (this._factories != null) {
                        this._factoryNames = new org.mwg.core.chunk.heap.ArrayLongLongMap(null, this._factories.length, null);
                    }
                    else {
                ...
                CoreGraph.prototype.newTypedNode = function (world, time, nodeType) {
                    ...
                    if (resolvedFactory == null) {
                        ...
                        newNode = resolvedFactory.create(world, time, this._nodeKeyCalculator.newKey(), this, initPreviouslyResolved);
                    }
                ...
                CoreGraph.prototype.factoryByCode = function (code) {
                    if (this._factoryNames != null && code != org.mwg.Constants.NULL_LONG) {
                        var resolvedFactoryIndex = this._factoryNames.get(code);
                        if (resolvedFactoryIndex != org.mwg.Constants.NULL_LONG) {
                            return this._factories[resolvedFactoryIndex];
                        }
                        else {
                            return null;
                        }
                                ...
                                selfPointer._storage.get(connectionKeys, function (payloads) {
                                    ...
                                    if (payloads != null) {
                                        ...
                                        try {
                                            ...
                                            var localFactories = selfPointer._factories;
                                            if (localFactories != null) {
                                                for (var i = 0; i < localFactories.length; i++) {
                                                    var encodedFactoryKey = selfPointer._resolver.stringToHash(localFactories[i].name(), false);
                ...
                CoreGraph.prototype.unindex = function (indexName, toIndexNode, flatKeyAttributes, callback) {
                    this.getIndexOrCreate(toIndexNode.world(), toIndexNode.time(), indexName, function (foundIndex) {
                        if (foundIndex != null) {
                            foundIndex.unindex(org.mwg.core.CoreConstants.INDEX_ATTRIBUTE, toIndexNode, flatKeyAttributes, function (result) {
                                foundIndex.free();
                                if (org.mwg.core.utility.PrimitiveHelper.isDefined(callback)) {
                                    callback(result);
                                }
                ...
                CoreGraph.prototype.find = function (world, time, indexName, query, callback) {
                    this.getIndexOrCreate(world, time, indexName, function (foundIndex) {
                        if (foundIndex == null) {
                            ...
                            foundIndex.findAt(org.mwg.core.CoreConstants.INDEX_ATTRIBUTE, world, time, query, function (collectedNodes) {
                                foundIndex.free();
                                if (org.mwg.core.utility.PrimitiveHelper.isDefined(callback)) {
                ...
                CoreGraph.prototype.all = function (world, time, indexName, callback) {
                    this.getIndexOrCreate(world, time, indexName, function (foundIndex) {
                        if (foundIndex == null) {
                            ...
                            foundIndex.allAt(org.mwg.core.CoreConstants.INDEX_ATTRIBUTE, world, time, function (collectedNodes) {
                                foundIndex.free();
                                if (org.mwg.core.utility.PrimitiveHelper.isDefined(callback)) {
                                    callback(collectedNodes);
                                }
                ...
                CoreGraph.prototype.actions = function () {
            ...
            var MWGResolver = (function () {
                ...
                MWGResolver.prototype.getOrLoadAndMarkAll = function (types, keys, callback) {
                    var nbKeys = keys.length / org.mwg.core.MWGResolver.KEY_SIZE;
                    var toLoadIndexes = [];
                    var nbElem = 0;
                    var result = new Array(nbKeys);
                    for (var i = 0; i < nbKeys; i++) {
                        if (keys[i * org.mwg.core.MWGResolver.KEY_SIZE] == org.mwg.core.CoreConstants.NULL_KEY[0] && keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 1] == org.mwg.core.CoreConstants.NULL_KEY[1] && keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 2] == org.mwg.core.CoreConstants.NULL_KEY[2]) {
                            toLoadIndexes[i] = false;
                            result[i] = null;
                        }
                        else {
                            result[i] = this._space.getAndMark(types[i], keys[i * org.mwg.core.MWGResolver.KEY_SIZE], keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 1], keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 2]);
                            if (result[i] == null) {
                    ...
                    if (nbElem == 0) {
                        ...
                        for (var i = 0; i < nbKeys; i++) {
                            if (toLoadIndexes[i]) {
                                ...
                                org.mwg.core.utility.BufferBuilder.keyToBuffer(keysToLoad, types[i], keys[i * org.mwg.core.MWGResolver.KEY_SIZE], keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 1], keys[i * org.mwg.core.MWGResolver.KEY_SIZE + 2]);
            ...
            (function (task_1) {
                    ...
                    ActionTraverseIndex.prototype.eval = function (context) {
                        ...
                        if (previousResult != null) {
                            ...
                            for (var i = 0; i < toLoad.length; i++) {
                                var node = toLoad[i];
                                if (this._query != null) {
                                    node.findAt(this._indexName, context.getWorld(), context.getTime(), this._query, function (result) {
                                    ...
                                    node.allAt(this._indexName, context.getWorld(), context.getTime(), function (result) {
                                        result.forEach(function (n) {
                                            if (n != null) {
                                                resultNodes[cursor.getAndIncrement()] = n;
                                            }
                ...
                var CoreTask = (function () {
                    ...
                    CoreTask.prototype.action = function (name, flatParams) {
                        var actionFactory = this._graph.actions().get(name);
                ...
                var CoreTaskActionRegistry = (function () {
                    ...
                    CoreTaskActionRegistry.prototype.add = function (name, action) {
                        this._factory.put(name, action);
                    };
                    CoreTaskActionRegistry.prototype.remove = function (name) {
                        this._factory.remove(name);
                    };
                    CoreTaskActionRegistry.prototype.get = function (name) {
                        return this._factory.get(name);
                    };
                    return CoreTaskActionRegistry;
                }());
                task_1.CoreTaskActionRegistry = CoreTaskActionRegistry;

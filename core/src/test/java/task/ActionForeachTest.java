...
public class ActionForeachTest extends AbstractActionTest {
    ...
    public void test() {
        ...
        graph.newTask()
                .from(new long[]{1, 2, 3})
                .foreach(graph.newTask().then(new KTaskAction() {
                    @Override
        ...
        graph.newTask().fromIndexAll("nodes").foreach(graph.newTask().then(new KTaskAction() {
            @Override
            public void eval(KTaskContext context) {
        ...
        graph.newTask().from(paramIterable).foreach(graph.newTask().then(new KTaskAction() {
            @Override
            public void eval(KTaskContext context) {
                context.setResult(context.getPreviousResult());
            }
